...
### 2026-05-31 20:55 JST - Codex - Travel score breakdown toggle

- Objective: 旅行Noteのカードでも撮影モジュールと同じようにスコア内訳を展開/折畳できるようにする。
- Files changed:
  - `assets/app.js`
  - `docs/AGENT_COLLABORATION_LOG.md`
- Summary:
  - 旅行カードに `スコア内訳` ボタンと `score-breakdown` を追加した。
  - 既存の `全て展開` / `全て折畳` ボタンを旅行カード表示でも使えるようにした。
  - 旅行データで個別スコアが未入力のスポットは、既存の推定ロジックから内訳スコアをランタイムで補完するようにした。
- Validation:
  - `node --check assets/app.js` 成功。
  - `curl -I http://127.0.0.1:4173/index.html` 成功。
  - 4173 で配信中の `assets/app.js` に今回の変更が含まれることを確認。
- Open items:
  - なし。
- Notes for next agent:
  - 旅行カードの内訳項目は `TRAVEL_SCORE` で管理している。
  - 撮影カードと同じ `[data-toggle-score]` / `.score-breakdown` のイベント処理を共有している。

### 2026-05-31 20:40 JST - Codex - Travel photo curation pass

- Objective: 旅行Noteで表示されていた誤マッチ写真（青梅の梅、食品、人物、駅名標、広告バナー等）を整理し、非Wikiの日本国内ソース中心に差し替える。
- Files changed:
  - `assets/travel_photo_meta.js`
  - `assets/photos/travel/*`
  - `curate_travel_photo_replacements.py`
  - `refetch_travel_photos_curated.py`
  - `docs/AGENT_COLLABORATION_LOG.md`
- Summary:
  - 125件の旅行写真を接触表で目視確認し、明らかな誤マッチを優先して39件差し替えた。
  - 差し替え元は県/市町村観光協会、景点公式、日本政府観光局、観光サイト等。Wiki/Wikimedia は引き続き禁止。
  - `curate_travel_photo_replacements.py` を追加し、手動確認済みの `travel_id -> source image` を再実行可能な形で管理するようにした。
  - 特に `T033 青梅・昭和レトロ` は果物写真を廃止し、青梅市観光協会由来の画像へ変更した。
- Validation:
  - `meta 125 missing 0 blocked 0` を確認。
  - `node --check assets/app.js` 成功。
  - `curl -I http://127.0.0.1:4173/index.html` が 200 OK。
  - in-app browser でローカルページをリロードし、表示中画像に broken image がないことを確認。
- Open items:
  - Flickr由来の画像はまだ一部残っている。今回の目的は「明らかな変な画像」の除去で、全125件の完全公式ソース化ではない。
  - `T033` は青梅市観光協会画像だが、昭和レトロ街並みとしてさらに良い写真が見つかれば次回差し替え候補。
- Notes for next agent:
  - 旅行写真を修正するときは `curate_travel_photo_replacements.py` に追記してから実行すること。
  - 実行後は `assets/travel_photo_meta.js` と `assets/photos/travel/` の両方が変わる。`localImageUrl` が存在するか必ず検証すること。
  - 画像取得では `wikipedia.org` / `wikimedia.org` を使わない。
